class InfoMessage:
    ...
    def get_message(self) -> str:
        ...
        Вам на проверку выскакивала ошибка: get_message() должна возвращать
        тип str, хотя def get_message(self) -> str: было прописано, решилась
        данная проблема только добавлением переменной message. Посмотрим
        пропустит ли проверка в этот раз ))."""
...
class Running(Training):
    ...
    ограничения строк <79 символов, я из-за этого ограничения и так
    несколько раз ошибалась в них, а после вывода констант строки становятся
    ещё длиннее."""
    K_1: int = 18
    K_2: int = 20
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        weight = self.weight
        cal_f_part2 = self.K_1 * weight + cal_f_part1 * self.K_2 * weight
        duration_in_min = self.duration * self.M_IN_H
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        calories: float = (speed + self.K_1) * self.K_2 * self.weight
        return calories
...
if __name__ == '__main__':
    ...
    """Сделала проверку на KeyError, Вы предлагали это сделать в
    read_package(), но там я не справилась с дальнейшей передачей
    данных в переменную training,в случае KeyError, поэтому решила
    сразу её проверять."""
    ...
    """Отдельное спасибо за комплименты в адрес пары решений в моём
    коде! )) Он мне дался совсем не просто во второй половине, а в Слак
    заглядывалать не хотелось, чтоб случайно чужое не списать."""

    """P.S. Напишите мне, пожалуйста, на будущее, если оставлять такие
    комментарии непосредственно в коде нельзя."""
